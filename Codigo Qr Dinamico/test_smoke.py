...
# --- allowed_connections ---
assert query_allowed_on({}, "Prod")                                # sin campo = todas
assert query_allowed_on({"allowed_connections": []}, "Prod")       # vacío = todas
assert query_allowed_on({"allowed_connections": ["Prod"]}, "Prod")
assert not query_allowed_on({"allowed_connections": ["Test"]}, "Prod")
...
for bad_field, bad_value in [
    ...
    bad = {**valid, "name": "Otra", bad_field: bad_value}
    try:
        store.add(bad)
...
bad_sql_auth = {**valid, "name": "SinUsuario", "auth_type": "sql_server", "username": ""}
try:
